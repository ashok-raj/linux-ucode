...
static void ucode_update_pointer(struct microcode_intel *mc)
{
...
static struct microcode_intel *__load_ucode_intel(struct ucode_cpu_info *uci, bool save, bool use_pa)
{
	...
	mc = scan_microcode(cp.data, cp.size, uci, save);

	return (use_pa ? (void *)__pa_nodebug(mc) : mc);
}

static int __init save_microcode_in_initrd_intel(void)
{
	struct ucode_cpu_info uci;
	 ...
	 * intel_ucode_patch_[pv]a. so the APs can use it for early loading.
	 */
	__load_ucode_intel(&uci, true, false);
	return 0;
}
...
void __init load_ucode_intel_bsp(void)
{
	...
	uci.mc = __load_ucode_intel(&uci, false, true);
	if (uci.mc)
